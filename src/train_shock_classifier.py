...
feature_cols = [c for c in df.columns if c not in ("date", "target_next_price", "pct_change", "is_shock")]
X = df[feature_cols]
y = df["is_shock"]
...
# Train final model on ALL data for saving/deployment
...
print("\nTop 10 features (final model, trained on all data):")
print(importance.head(10))

final_model.save_model("db/shock_classifier.json")
print("\nSaved final model to db/shock_classifier.json")
